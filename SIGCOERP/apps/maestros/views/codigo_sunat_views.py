...
from ..forms.codigo_sunat_forms import CodigoSunatForm


modelo = CodigoSunat
modelo_string = "codigo_sunat"
formulario = CodigoSunatForm
...
class CodigoSunatListView(GenericListView):
	model = modelo
	context_object_name = 'datos'
	template_name = f"maestros/{url_list}.html"
	cadena_filtro = "Q(codigo_sunat__icontains=text) | \
					 Q(descripcion_codigo_sunat__icontains=text)"
	extra_context = {
...
# @method_decorator(login_required, name='dispatch')
class CodigoSunatCreateView(GenericCreateView):
	model = modelo
...
# @method_decorator(login_required, name='dispatch')
class CodigoSunatUpdateView(GenericUpdateView):
	model = modelo
	form_class = formulario
	template_name = f"maestros/{plantilla_form}"
	success_url = reverse_lazy(url_list) # Nombre de la url.
...
# @method_decorator(login_required, name='dispatch')
class CodigoSunatDeleteView(GenericDeleteView):
	model = modelo
	...
	extra_context = {
		...
		"url_list" : url_list,
		"mensaje": "Estás seguro que deseas eliminar el Código SUNAT"
	}
# ------------------------------------------------------------------------------
