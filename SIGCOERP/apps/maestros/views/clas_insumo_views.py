...
from ..forms.clas_insumo_forms import ClasInsumoForm


modelo = ClasInsumo
modelo_string = "clas_insumo"
formulario = ClasInsumoForm
...
class ClasInsumoListView(GenericListView):
	model = modelo
	context_object_name = 'datos'
	template_name = f"maestros/{url_list}.html"
	cadena_filtro = "Q(nombre_familia_insumo__icontains=text) | \
...
# @method_decorator(login_required, name='dispatch')
class ClasInsumoCreateView(GenericCreateView):
	model = modelo
...
# @method_decorator(login_required, name='dispatch')
class ClasInsumoUpdateView(GenericUpdateView):
	model = modelo
	form_class = formulario
	template_name = f"maestros/{plantilla_form}"
	success_url = reverse_lazy(url_list) # Nombre de la url.
...
# @method_decorator(login_required, name='dispatch')
class ClasInsumoDeleteView(GenericDeleteView):
	model = modelo
	...
	extra_context = {
		...
		"url_list" : url_list,
		"mensaje": "Estás seguro de que deseas eliminar la Familia Insumo"
	}
# ------------------------------------------------------------------------------
