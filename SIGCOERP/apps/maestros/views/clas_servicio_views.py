...
from ..forms.clas_servicio_forms import ClasServicioForm


modelo = ClasServicio
modelo_string = "clas_servicio"
formulario = ClasServicioForm
...
class ClasServicioListView(GenericListView):
	model = modelo
	context_object_name = 'datos'
	template_name = f"maestros/{url_list}.html"
	cadena_filtro = "Q(nombre_familia_servicio__icontains=text) | \
...
# @method_decorator(login_required, name='dispatch')
class ClasServicioCreateView(GenericCreateView):
	model = modelo
...
# @method_decorator(login_required, name='dispatch')
class ClasServicioUpdateView(GenericUpdateView):
	model = modelo
	form_class = formulario
	template_name = f"maestros/{plantilla_form}"
	success_url = reverse_lazy(url_list) # Nombre de la url.
...
# @method_decorator(login_required, name='dispatch')
class ClasServicioDeleteView(GenericDeleteView):
	model = modelo
	...
	extra_context = {
		...
		"url_list" : url_list,
		"mensaje": "Estás seguro de que deseas eliminar la Familia Servicio"
	}
# ------------------------------------------------------------------------------
