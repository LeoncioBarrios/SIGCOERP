...
class MonedaListView(GenericListView):
	model = modelo
	context_object_name = 'datos'
	template_name = f"maestros/{url_list}.html"
	cadena_filtro = "Q(nombre_moneda__icontains=text) | \
					 Q(simbolo_moneda__icontains=text) | \
					 Q(descripcion_moneda__icontains=text)"
	extra_context = {
...
# @method_decorator(login_required, name='dispatch')
class MonedaCreateView(GenericCreateView):
	model = modelo
...
# @method_decorator(login_required, name='dispatch')
class MonedaUpdateView(GenericUpdateView):
	model = modelo
	form_class = formulario
	template_name = f"maestros/{plantilla_form}"
	success_url = reverse_lazy(url_list) # Nombre de la url.
...
# @method_decorator(login_required, name='dispatch')
class MonedaDeleteView(GenericDeleteView):
	model = modelo
	...
	extra_context = {
		...
		"url_list" : url_list,
		"mensaje": "Estás seguro de que deseas eliminar la Moneda"
	}
# ------------------------------------------------------------------------------
