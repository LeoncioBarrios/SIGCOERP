...
from ..forms.ubigeo_forms import UbigeoForm


modelo = Ubigeo
modelo_string = modelo.__name__.lower()
formulario = UbigeoForm
...
class UbigeoListView(GenericListView):
	model = modelo
	context_object_name = 'datos'
	template_name = f"maestros/{url_list}.html"
	cadena_filtro = "Q(ubigeo__icontains=text) | \
					 Q(departamento__icontains=text) | \
					 Q(provincia__icontains=text) | \
					 Q(distrito__icontains=text)"
	extra_context = {
...
# @method_decorator(login_required, name='dispatch')
class UbigeoCreateView(GenericCreateView):
	model = modelo
...
# @method_decorator(login_required, name='dispatch')
class UbigeoUpdateView(GenericUpdateView):
	model = modelo
	form_class = formulario
	template_name = f"maestros/{plantilla_form}"
	success_url = reverse_lazy(url_list) # Nombre de la url.
...
# @method_decorator(login_required, name='dispatch')
class UbigeoDeleteView(GenericDeleteView):
	model = modelo
	...
	extra_context = {
		...
		"url_list" : url_list,
		"mensaje": "Estás seguro de que deseas eliminar el Ubigeo"
	}
# ------------------------------------------------------------------------------
