...
modelo = FormaPago
modelo_string = "forma_pago"
formulario = FormaPagoForm
...
class FormaPagoListView(GenericListView):
	model = modelo
	context_object_name = 'datos'
	template_name = f"maestros/{url_list}.html"
	cadena_filtro = "Q(nombre_forma_pago__icontains=text) | \
					 Q(codigo_forma_pago__icontains=text)"
	extra_context = {
...
# @method_decorator(login_required, name='dispatch')
class FormaPagoCreateView(GenericCreateView):
	model = modelo
...
# @method_decorator(login_required, name='dispatch')
class FormaPagoUpdateView(GenericUpdateView):
	model = modelo
	form_class = formulario
	template_name = f"maestros/{plantilla_form}"
	success_url = reverse_lazy(url_list) # Nombre de la url.
...
# @method_decorator(login_required, name='dispatch')
class FormaPagoDeleteView(GenericDeleteView):
	model = modelo
	...
	extra_context = {
		...
		"url_list" : url_list,
		"mensaje": "Estás seguro de que deseas eliminar la Forma de Pago"
	}
# ------------------------------------------------------------------------------
