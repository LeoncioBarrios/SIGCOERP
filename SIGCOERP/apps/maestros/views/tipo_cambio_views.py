...
from ..forms.tipo_cambio_forms import TipoCambioForm


modelo = TipoCambio
modelo_string = "tipo_cambio"
...
class TipoCambioListView(GenericListView):
	model = modelo
	context_object_name = 'datos'
	template_name = f"maestros/{url_list}.html"
	cadena_filtro = "Q(id_moneda__nombre_moneda__icontains=text)"
	extra_context = {
...
# @method_decorator(login_required, name='dispatch')
class TipoCambioCreateView(GenericCreateView):
	model = modelo
...
# @method_decorator(login_required, name='dispatch')
class TipoCambioUpdateView(GenericUpdateView):
	model = modelo
	form_class = formulario
	template_name = f"maestros/{plantilla_form}"
	success_url = reverse_lazy(url_list) # Nombre de la url.
...
# @method_decorator(login_required, name='dispatch')
class TipoCambioDeleteView(GenericDeleteView):
	model = modelo
	...
	extra_context = {
		...
		"url_list" : url_list,
		"mensaje": "Estás seguro de que deseas eliminar el Tipo de Cambio"
	}
# ------------------------------------------------------------------------------
