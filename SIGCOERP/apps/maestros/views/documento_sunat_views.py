...
from ..forms.documento_sunat_forms import DocumentoSunatForm

modelo = DocumentoSunat
modelo_string = "documento_sunat"
formulario = DocumentoSunatForm
...
class DocumentoSunatListView(GenericListView):
	model = modelo
	context_object_name = 'datos'
	template_name = f"maestros/{url_list}.html"
	cadena_filtro = "Q(nombre_documento_sunat__icontains=text) | \
...
# @method_decorator(login_required, name='dispatch')
class DocumentoSunatCreateView(GenericCreateView):
	model = modelo
...
# @method_decorator(login_required, name='dispatch')
class DocumentoSunatUpdateView(GenericUpdateView):
	model = modelo
	form_class = formulario
	template_name = f"maestros/{plantilla_form}"
	success_url = reverse_lazy(url_list) # Nombre de la url.
...
# @method_decorator(login_required, name='dispatch')
class DocumentoSunatDeleteView(GenericDeleteView):
	model = modelo
	...
	extra_context = {
		...
		"url_list" : url_list,
		"mensaje": "Estás seguro que deseas eliminar el Documento SUNAT"
	}
# ------------------------------------------------------------------------------
